...
train <- train[which(train$Species=="CAT"),]
test <- test[which(test$Species=="CAT"),]
...
levels(train$Primary.Breed) <- 1:26
levels(test$Primary.Breed) <- 1:26
levels(train$Color.Markings) <- 1:293
levels(test$Color.Markings) <- 1:293
...
train$Primary.Breed = NULL
test$Primary.Breed = NULL
...
xgb.plot.importance(importance_matrix[1:22,])

#############################################################
write.csv(pred1,"CAT222.csv", row.names = FALSE)



table(train$ml, train$OutCatg)
